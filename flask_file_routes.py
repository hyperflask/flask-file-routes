...
def register_page_package(app_or_blueprint, path="pages", package_name=None, template_prefix=None, url_prefix="", skip_missing_frontmatter=False, jinja_env=None):
    ...
    for module_name, template in loader.list_files(module_with_package=False):
        ...
        page_module = ModuleView.create_from_module_template(loader, module_name, template, url, skip_missing_frontmatter)
        if page_module:
...
class ModuleView:
    ...
    def create_from_module_template(cls, loader, module_name, template, url=None, skip_missing_frontmatter=False):
        if template:
            ...
            template_code = None
            python_code = loader.split_source(loader.get_template_from_pymodule(module_name))[1]
        ...
        if python_code:
            ...
            if m:
                ...
                methods = [m.upper() for m in verbs if re.search(f"^(def {m}\()|{m}\s*=", python_code, re.MULTILINE)]
            if not methods:
                methods = ["GET"]
        else:
            methods = ["GET"]
        ...
        return cls(module_name, template, url, methods)

    def __init__(self, module_name, template, url=None, methods=None, decorators=None):
        if not methods:
            methods = ["GET"]
    ...
    def execute(self, view_args=None, method=None):
        ...
        if out is None and page.template:
            ...
            if page.template.endswith(".md") or page.get('is_markdown'):
                out = markdown.markdown(out, **current_app.config.get("PAGES_MARKDOWN_OPTIONS", {}))
                if page.get("layout"):
                    layout = page.layout
                    block = "content"
                    if ":" in layout:
